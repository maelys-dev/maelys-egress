...
```sh
tar -xzf maelys-egress-node-sdk-0.19.13.tar.gz
npm install ./maelys-egress-node-sdk-0.19.13
```
...
rule is skipped and named in the final error. A process running with more
privileges than the owner of a Homebrew prefix therefore never picks the
binary from that prefix; it must pass an explicit `binary`. An explicit
`binary` is trusted as given; `binaryTrustRefusal(path)` returns why a path
fails the same rule, or `null`, for callers that want to apply it themselves.
